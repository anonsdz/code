...
TOKEN = '7918986015:AAEFfQ7GWkUI22QZI8QFpRoEEcDiVLw6df8'

# Danh sách các ID của admin
ADMIN_IDS = [7371969470]  # Chỉ có một admin cố định, các ID khác có thể thêm sau.




# Danh sách các GROUP_ID mà bot được phép hoạt động
GROUP_IDS = [-1002494586692, -1002177463101]  # Thêm các GROUP_ID vào danh sách này
...
async def attack_method(update, context, methods_data):
    ...
    if not attack_time:
        return await update.message.reply_text(f"Thời gian Attacker của phương thức {method_name} không được thiết lập trong methods.json.")
